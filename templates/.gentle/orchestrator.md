...
Before launching ANY sub-agent, you MUST dynamically inject its context. DO NOT guess or hardcode skills.

### Step 1: Resolve Phase & Skill

Read `.gentle/context-protocol.md` to find the exact skill pointer file mapped to your current SDD phase. Read the content of that specific pointer file.

### Step 2: Resolve Standards (If applicable)

Read `.gentle/sdd.yaml` to find the `conventions` active for this project.
If the sub-agent will write, design, or analyze code (e.g., `design`, `apply`, `verify`), read the corresponding `.gentle/standards/{convention}.md` files.

### Step 3: Compose Sub-Agent Prompt

Construct the delegation prompt strictly as follows:

```
{content of the skill pointer file}

--- STANDARDS (if applicable) ---
{content of active standards from Step 2}

--- CONTEXT ---
{relevant context from engram or previous SDD phases}
```

### Step 4: Delegate

Launch the sub-agent with the composed prompt using `task` (sync) or `delegate` (async) according to the Delegation Rules.
